...
double midpoint(struct integral I) {
  double ans;
  double a = I.low, b = I.high;
  ans = I.integrand((a+b)/2)*(b-a); //height times base
  return ans;
}
...
double simpson_13(struct integral I) {
  double ans, simp;
  double a = I.low, b = I.high;
  simp = I.integrand(a)+4*I.integrand((a+b/2))+I.integrand(b); //quadratic portion
...
double simpson_38(struct integral I) {
  double ans, simp;
  double a = I.low, b = I.high;
  simp = I.integrand(a)+3*I.integrand((2*a+b)/3)+3*I.integrand((a+2*b)/3)+I.integrand(b); //cubic portion
  ans = (b-a)/8*simp; //horizontal portion
  return ans;
}

//approximates integral via guass quad method; requires 4th input of type integer
double gauss_quad(struct integral I,int n) {
